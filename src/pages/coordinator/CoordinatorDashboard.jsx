...
const NAV_PAGES = [
  { label: 'Application Review',   sub: 'Review and decide on applicants',    accent: T.blue,    path: '/coordinator/applications', icon: <ClipboardIcon /> },
  { label: 'Evaluation Tracking',  sub: 'Monitor employer evaluations',        accent: T.success, path: '/coordinator/evaluations',  icon: <ChartIcon /> },
...
export default function CoordinatorDashboard() {
  ...
  const [stats, setStats] = useState({ total: 0, provisional: 0, finalAccepted: 0, rejected: 0, pendingReports: 0, pendingEvals: 0 });
  ...
  async function fetchStats() {
    try {
      ...
      setStats({
        ...
        finalAccepted: apps.filter(a => a.status === 'accepted').length,
  ...
  const STATS = [
    { label: 'Total Applications',     value: stats.total,         accent: T.blue   },
    { label: 'Provisionally Accepted', value: stats.provisional,   accent: T.yellow },
    { label: 'Finally Accepted',       value: stats.finalAccepted, accent: T.success},
    ...
    <div style={{ minHeight: '100vh', background: T.pageBg, fontFamily: "'Inter', 'Segoe UI', sans-serif" }}>
      ...
      <nav style={{ backgroundColor: '#fff', borderBottom: `1px solid ${T.cardBorder}`, padding: '0 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <span style={{ fontSize: '1rem', fontWeight: '700', color: T.textDark }}>Co-op Coordinator Portal</span>
        </div>
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '2rem' }}>
        ...
        {/* Alert banner */}
        {(stats.pendingReports > 0 || stats.pendingEvals > 0) && (
          <div style={{ backgroundColor: 'rgba(217,119,6,0.07)', border: `1px solid rgba(217,119,6,0.25)`, borderRadius: '10px', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <button onClick={() => navigate('/coordinator/reporting')} style={{ padding: '0.45rem 1rem', borderRadius: '7px', border: `1.5px solid rgba(217,119,6,0.35)`, backgroundColor: 'rgba(217,119,6,0.1)', color: T.warning, fontSize: '0.82rem', fontWeight: '600', fontFamily: 'inherit', cursor: 'pointer' }}>
              Send Reminders
            </button>
          </div>
        )}
