...
function UploadSection({ studentId, existingReport, onUploadSuccess }) {
  ...
  async function handleFile(file) {
    ...
    const path = `reports/${studentId}/${Date.now()}_${file.name}`
    const storageRef = ref(storage, path)
    const task = uploadBytesResumable(storageRef, file)

    task.on('state_changed',
      snap => setProgress(Math.round((snap.bytesTransferred / snap.totalBytes) * 100)),
      () => { setUploadState('error'); setErrorMsg('Upload failed. Please try again.') },
      async () => {
        const url = await getDownloadURL(task.snapshot.ref)
        onUploadSuccess({ url, name: file.name, uploadedAt: new Date() })
        setUploadState('done')
      }
    )
  }
  ...
  if (existingReport) {
    return (
      <div style={{ backgroundColor: T.successBg, border: `1.5px solid ${T.successBorder}`, borderRadius: '10px', padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
        ...
        <a href={existingReport.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.8rem', color: T.success, fontWeight: '600', textDecoration: 'underline' }}>
          View
        </a>
      </div>
    )
    ...
    <div>
      <label
        ...
        style={{
          ...
          border: `2px dashed ${dragOver ? T.blue : uploadState === 'error' ? T.danger : '#c5daf0'}`,
          borderRadius: '10px', cursor: 'pointer', transition: 'all 0.15s',
          backgroundColor: dragOver ? T.blueLight : uploadState === 'error' ? T.dangerBg : '#f8fafc',
        }}
      >
        ...
        {uploadState === 'error' && (
          <span style={{ fontSize: '0.88rem', fontWeight: '600', color: T.danger }}>{errorMsg}</span>
        )}
...
export default function StudentDashboard() {
  ...
  async function fetchData() {
    try {
      ...
      const reportsQ = query(collection(db, 'reports'), where('studentId', '==', currentUser.uid))
      const reportsSnap = await getDocs(reportsQ)
      if (!reportsSnap.empty) setReport(reportsSnap.docs[0].data())
    } catch (err) {
      console.error(err)
    } finally {
      setLoading(false)
  ...
  function handleUploadSuccess(reportData) {
    ...
    <div style={{ minHeight: '100vh', background: T.pageBg, fontFamily: "'Inter', 'Segoe UI', sans-serif" }}>
      ...
      <nav style={{ backgroundColor: '#fff', borderBottom: `1px solid ${T.cardBorder}`, padding: '0 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ width: '32px', height: '32px', borderRadius: '8px', backgroundColor: '#eef2fb', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={T.blue} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span style={{ fontSize: '0.82rem', color: T.textMuted, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto', padding: '2rem' }}>
        ...
        <div style={{ marginBottom: '1.75rem' }}>
          <h1 style={{ fontSize: '1.6rem', fontWeight: '700', color: T.textDark, margin: 0 }}>
            Welcome back{student?.name ? `, ${student.name.split(' ')[0]}` : ''}
          </h1>
          <p style={{ fontSize: '0.9rem', color: T.textMuted, marginTop: '0.3rem' }}>
            Here is your co-op application overview.
          </p>
        </div>
        ...
        {application?.reportDeadline && (
          <div style={{ marginTop: '1rem' }}>
            <DeadlineBanner deadlineDate={application.reportDeadline} />
          </div>
        )}
        ...
        <div style={{ backgroundColor: T.card, border: `1px solid ${T.cardBorder}`, borderRadius: '12px', padding: '1.5rem', boxShadow: T.cardShadow, marginTop: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
            ...
            {!canUpload && (
              <span style={{ padding: '0.25rem 0.75rem', borderRadius: '20px', fontSize: '0.72rem', fontWeight: '700', backgroundColor: '#f1f3f7', border: '1px solid #dde3ed', color: T.textMuted, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                Locked
              </span>
            )}
          </div>
          ...
          {canUpload ? (
            ...
            <div style={{ padding: '1.5rem', backgroundColor: '#f8fafc', border: `1px solid ${T.cardBorder}`, borderRadius: '8px', textAlign: 'center' }}>
              ...
              <p style={{ color: T.textMuted, fontSize: '0.85rem', margin: 0 }}>
                Upload will be unlocked after your application is accepted.
              </p>
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '1.5rem' }}>
          ...
          <div style={{ backgroundColor: T.card, border: `1px solid ${T.cardBorder}`, borderRadius: '12px', padding: '1.25rem 1.5rem', boxShadow: T.cardShadow }}>
            ...
            {status === 'pending' && (
              <p style={{ fontSize: '0.85rem', color: T.textMid, lineHeight: 1.6, margin: 0 }}>
                Your application is under review. The co-op coordinator will update your status once a decision has been made. Check back soon.
              </p>
            )}
            {status === 'provisional' && (
              <p style={{ fontSize: '0.85rem', color: T.textMid, lineHeight: 1.6, margin: 0 }}>
                You have been provisionally accepted. The coordinator will verify your eligibility before giving final confirmation. Keep an eye on your email.
              </p>
            )}
            {status === 'accepted' && (
              <p style={{ fontSize: '0.85rem', color: T.textMid, lineHeight: 1.6, margin: 0 }}>
                You are confirmed in the co-op program. Please upload your work term report before the deadline using the upload section on this page.
              </p>
            )}
            {status === 'rejected' && (
              <p style={{ fontSize: '0.85rem', color: T.textMid, lineHeight: 1.6, margin: 0 }}>
                Your application was not successful this term. Please contact the co-op coordinator for guidance on next steps or reapplying in a future term.
              </p>
            )}
          </div>
        </div>
